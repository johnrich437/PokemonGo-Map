...
def retrying_set_location(location_name):
    ...
    while True:
        try:
            FLOAT_LAT, FLOAT_LONG = set_location(location_name)
...
def set_location_coords(lat, long, alt, o_id):
    COORDS_LATITUDE = f2i(lat)  # 0x4042bd7c00000000 # f2i(lat)
    COORDS_LONGITUDE = f2i(long)  # 0xc05e8aae40000000 #f2i(long)
    COORDS_ALTITUDE = f2i(alt)
    collection.update_one({'_id': o_id}, {
        '$set': {'COORDS_LATITUDE': COORDS_LATITUDE, 'COORDS_LONGITUDE': COORDS_LONGITUDE,
                 'COORDS_ALTITUDE': COORDS_ALTITUDE}})


def get_location_coords(o_id):
    obj = collection.find_one({'_id': o_id})

    return (obj['COORDS_LATITUDE'], obj['COORDS_LONGITUDE'], obj['COORDS_ALTITUDE'])


def retrying_api_req(service, api_endpoint, access_token, o_id, *args, **kwargs):
    while True:
        try:
            response = api_req(service, api_endpoint, access_token, o_id, *args,
                               **kwargs)
            if response:
                return response
...
def api_req(service, api_endpoint, access_token, o_id, *args, **kwargs):
...
def get_api_endpoint(service, access_token, o_id, api=API_URL):
...
def retrying_get_profile(service, access_token, api, useauth, o_id, *reqq):
    profile_response = None
    while not profile_response:
        profile_response = get_profile(service, access_token, api, useauth, o_id,
                                       *reqq)
...
def get_profile(service, access_token, api, useauth, o_id, *reqq):
    ...
    return retrying_api_req(service, api, access_token, o_id, req, useauth=useauth)
...
def get_heartbeat(service,
                  api_endpoint,
                  access_token,
                  response, FLOAT_LAT, FLOAT_LONG, o_id):
...
def get_args():
    ...
    parser.add_argument(
        ...
        help="Enables an autorefresh that behaves the same as a page reload. " +
             "Needs an integer value for the amount of seconds", default="1")
    parser.add_argument(
        '-dp',
        '--display-pokestop',
...
@memoize
def login(args, o_id):
    access_token = get_token(args.auth_service, "johnjosh1927@gmail.com", "MyNameIsEduard")
    if access_token is None:
        raise Exception('[-] Wrong username/password')

    print '[+] RPC Session Token: {} ...'.format(access_token[:25])

    api_endpoint = get_api_endpoint(args.auth_service, access_token, o_id)
...
def main(api_endpoint, access_token, profile_response, pokemonsJSON, o_id):
    obj = collection.find_one({'_id': o_id})
    ...
    for step in range(steplimit2):
        ...
        if -steplimit2 / 2 < x <= steplimit2 / 2 and -steplimit2 / 2 < y <= steplimit2 / 2:
            set_location_coords(x * 0.0025 + obj['lat'], y * 0.0025 + obj['lng'], 0, o_id)
        ...
        process_step(args, api_endpoint, access_token, profile_response,
                     pokemonsJSON, ignore, only, o_id)

        print('Completed: ' + str(
            ((step + 1) + pos * .25 - .25) / (steplimit2) * 100) + '%')

    set_location_coords(obj['lat'], obj['lng'], 0, o_id)

    register_background_thread(api_endpoint, access_token, profile_response, pokemonsJSON, o_id)
...
def process_step(args, api_endpoint, access_token, profile_response,
    ...
    h = get_heartbeat(args.auth_service, api_endpoint, access_token,
                      profile_response, FLOAT_LAT, FLOAT_LONG, o_id)
    hs = [h]
    seen = set([])

    for child in parent.children():
        latlng = LatLng.from_point(Cell(child).get_center())
        set_location_coords(latlng.lat().degrees, latlng.lng().degrees, 0, o_id)
        hs.append(
            get_heartbeat(args.auth_service, api_endpoint, access_token,
                          profile_response, FLOAT_LAT, FLOAT_LONG, o_id))
    set_location_coords(step_lat, step_long, 0, o_id)
    visible = []
                    ...
                    for Fort in cell.Fort:
                        if Fort.Enabled == True:
                            ...
                            if Fort.GymPoints and args.display_gym:
                                gyms[Fort.FortId.replce('.', '')] = [Fort.Team, Fort.Latitude,
                                ...
                                if (expire_time != 0 or not args.onlylure):
                                    pokestops[Fort.FortId.replce('.', '')] = [Fort.Latitude,
    ...
    collection.update_one({'_id': ObjectId(ob_id_str)}, {'$set': {'gyms': gyms}})
    collection.update_one({'_id': ObjectId(ob_id_str)}, {'$set': {'pokemons': pokemons}})
    collection.update_one({'_id': ObjectId(ob_id_str)}, {'$set': {'pokestops': pokestops}})


def register_background_thread(api_endpoint, access_token, profile_response, pokemonsJSON, o_id):
    """
    ...
    search_thread = threading.Thread(target=main, args=(
        api_endpoint, access_token, profile_response, pokemonsJSON, o_id))
...
@app.route('/data')
def data():
    o_id = flask.request.args.get('o_id', '')
    if o_id != '':
        obj = collection.find_one({'_id': ObjectId(o_id)})
        """ Gets all the PokeMarkers via REST """
        return json.dumps(get_pokemarkers(obj['lat'], obj['lng'], obj['pokemons'], obj['gyms'], obj['pokestops']))
    else:
        return json.dumps({})
...
def config():
    ...
    obj = collection.find_one({'_id': ObjectId(flask.request.args.get('o_id', ''))})
...
def fullmap():
    fullmap = Map(
        identifier="fullmap",
        style='height:80%;width:100%;top:0;left:0;position:absolute;z-index:200;',
        lat=0.0,
        lng=0.0,
        zoom='3')
    return render_template(
        'example_fullmap.html', key=GOOGLEMAPS_KEY, fullmap=fullmap,
        auto_refresh=auto_refresh, obj_id='')
...
def search():
    ...
    FLOAT_LAT, FLOAT_LONG = retrying_set_location(query)
    o_id = collection.insert(
        {'lat': FLOAT_LAT, 'lng': FLOAT_LONG, 'gyms': {}, 'pokemons': {}, 'pokestops': {}, 'COORDS_LATITUDE': 0,
         'COORDS_LONGITUDE': 0, 'COORDS_ALTITUDE': 0})
    ...
    if args.ampm_clock:
        ...
        is_ampm_clock = True
    api_endpoint, access_token, profile_response = login(args, o_id)
    api_endpoint = get_api_endpoint(args.auth_service, access_token, o_id)

    register_background_thread(api_endpoint, access_token, profile_response, pokemonsJSON, o_id)
    options = {'identifier': 'fullmap', 'lat': FLOAT_LAT, 'lng': FLOAT_LONG, 'zoom': 15}
    return render_template(
        'example_fullmap.html', key=GOOGLEMAPS_KEY, fullmap=get_map(o_id),
        auto_refresh=auto_refresh, obj_id=o_id, options=options)


def get_pokemarkers(lat, lon, pokemons, gyms, pokestops):
    pokeMarkers = [{
        'icon': icons.dots.red,
        'lat': lat,
        'lng': lon,
...
def get_map(o_id):
    ...
    fullmap = Map(
        identifier="fullmap",
        style='height:80%;width:100%;bottom:0;left:0;position:absolute;z-index:200;',
        lat=obj['lat'],
        lng=obj['lng'],
        markers=get_pokemarkers(obj['lat'], obj['lng'], obj['pokemons'], obj['gyms'], obj['pokestops']),
        zoom='15')
    return fullmap
